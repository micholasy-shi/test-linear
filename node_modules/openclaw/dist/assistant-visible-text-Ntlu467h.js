...
//#endregion
//#region src/shared/text/plain-text-tool-call-blocks.ts
const DEFAULT_MAX_PAYLOAD_BYTES = 256e3;
...
function parseBlockAt(text, start, options) {
	...
	const payload = consumeJsonObject(text, opening.end, options?.maxPayloadBytes ?? DEFAULT_MAX_PAYLOAD_BYTES);
...
function stripPlainTextToolCallBlocks(text) {
	...
	while (index < text.length) {
		...
		const block = parseBlockAt(text, skipHorizontalWhitespace(text, index));
		if (!block) {
			index += 1;
			continue;
		}
...
//#endregion
export { stripDowngradedToolCallText as a, stripModelSpecialTokens as c, findCodeRegions as d, isInsideCode as f, stripAssistantInternalScaffolding as i, hasOrphanReasoningCloseBoundary as l, sanitizeAssistantVisibleTextWithOptions as n, stripMinimaxToolCallXml as o, sanitizeAssistantVisibleTextWithProfile as r, stripToolCallXmlTags as s, sanitizeAssistantVisibleText as t, stripReasoningTagsFromText as u };

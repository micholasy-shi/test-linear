...
import { t as extractBalancedJsonFragments } from "./balanced-json-CMmZ32XQ.js";
...
function closeClaudeLiveSessionForContext(context) {
	const key = buildClaudeLiveKey(context);
	const session = liveSessions.get(key);
	if (session) closeLiveSession(session, "restart");

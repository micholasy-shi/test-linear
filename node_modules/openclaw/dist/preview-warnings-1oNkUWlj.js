...
function loadChannelDoctorModule() {
	channelDoctorModulePromise ??= import("./channel-doctor-BnLhwCBB.js");
...
async function collectDoctorPreviewWarnings(params) {
	...
	const channelPluginRuntime = hasChannelConfig && hasExplicitChannelPluginBlockerConfig(params.cfg) ? await import("./channel-plugin-blockers-BxJtldJ-.js") : void 0;
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-2mGKmuOl.js");
	...
	if (hasPluginConfig || hasChannelConfig) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-KYDvB3rY.js");
	...
	if (hasPluginConfig) {
		const { collectCodexRouteWarnings } = await import("./codex-route-warnings-Dsaqkj9O.js");
	...
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-Bm8bR1HP.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-DDqc4Edl.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-CGU40dNI.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-YECQ7Nhn.js");

import { T as isValueToken, a as getPositiveIntFlagValue, c as hasFlag, i as getFlagValue, r as getCommandPositionalsWithRootOptions, s as getVerboseFlag } from "./argv-Bh2LLxMT.js";
...
import { n as matchesCommandPath, r as cliCommandCatalog, t as resolveCliCommandPathPolicy } from "./command-path-policy-DRdOarWv.js";
import { n as ensureCliExecutionBootstrap, r as resolveCliExecutionStartupContext, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-Bao9Epfx.js";
...
function loadModelsListCommand() {
	modelsListCommandPromise ??= import("./list.list-command-C_AqUina.js");
	return modelsListCommandPromise;
}
function loadModelsStatusCommand() {
	modelsStatusCommandPromise ??= import("./list.status-command-CHsNisGK.js");
...
const routedCommandDefinitions = {
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-Ch1zUh4a.js");
			...
			const { statusCommand } = await import("./status-jo-gcsxv.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-VrP6Dps1.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-CMPiKpxU.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-Cifr7IAK.js");
	...
	"tasks-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { tasksListJsonCommand } = await import("./tasks-json-VTYaJprQ.js");
	...
	"tasks-audit": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { tasksAuditJsonCommand } = await import("./tasks-json-VTYaJprQ.js");
	...
	"channels-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsListCommand } = await import("./list-D68fhlq3.js");
	...
	"channels-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsStatusCommand } = await import("./status-CDu-UlAq.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-CADiJ9_Z.js");

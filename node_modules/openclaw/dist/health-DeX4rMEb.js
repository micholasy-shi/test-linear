...
import { n as buildGatewayConnectionDetails, r as callGateway } from "./call-CP7A3sdw.js";
import { t as listReadOnlyChannelPluginsForConfig } from "./read-only-DoF9vWKk.js";
import { a as resolvePreferredAccountId, t as buildChannelAccountBindings } from "./bindings-C4DZxyB0.js";
...
import { i as projectSafeChannelAccountSnapshotFields } from "./account-snapshot-fields-DMBPyKXj.js";
...
const buildSessionSummary = async (storePath) => {
	const { loadSessionStore } = await import("./store-BFDdHd51.js");
...
async function getHealthSnapshot(params) {
	...
	for (const plugin of plugins) {
		...
		for (const accountId of accountIdsToProbe) {
			...
			const snapshot = {
				...projectSafeChannelAccountSnapshotFields(params?.runtimeSnapshot?.channelAccounts[plugin.id]?.[accountId] ?? (accountId === defaultAccountId ? params?.runtimeSnapshot?.channels[plugin.id] : void 0)),
				accountId,
				enabled,
				configured
			};
			if (includeSensitive && probe !== void 0) snapshot.probe = probe;

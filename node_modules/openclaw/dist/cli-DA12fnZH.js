import { a as fetchGoogleMeetSpace, i as fetchGoogleMeetAttendance, n as createGoogleMeetSpace, o as fetchLatestGoogleMeetConferenceRecord, r as fetchGoogleMeetArtifacts, t as buildGoogleMeetPreflightReport } from "./meet-cl7wP0I6.js";
...
import { a as buildGoogleMeetAuthUrl, c as exchangeGoogleMeetAuthCode, f as waitForGoogleMeetAuthCode, o as createGoogleMeetOAuthState, s as createGoogleMeetPkce, u as resolveGoogleMeetAccessToken } from "./oauth-BwCXCvyY.js";
...
function registerGoogleMeetCli(params) {
	...
	root.command("setup").description("Show Google Meet transport setup status").option("--transport <transport>", "Transport to check: chrome, chrome-node, or twilio").option("--json", "Print JSON output", false).action(async (options) => {
		const status = await (await params.ensureRuntime()).setupStatus({ transport: options.transport });
		if (options.json) {
			writeStdoutJson(status);
			return;
		}

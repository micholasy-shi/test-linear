...
import { i as prepareWhatsAppOutboundMedia, o as sendWhatsAppOutboundWithRetry, r as normalizeWhatsAppPayloadTextPreservingIndentation, t as normalizeWhatsAppOutboundPayload } from "./outbound-media-contract-DuGCJRLW.js";
import { r as sendReactionWhatsApp } from "./send-DcMGZ9qO.js";
...
import { _ as readWebSelfIdentityForDecision, a as getWebAuthAgeMs, h as readWebSelfId, r as WhatsAppAuthUnstableError } from "./auth-store-Da7U_n_T.js";
...
import { c as waitForWaConnection, d as newConnectionId, f as resolveHeartbeatSeconds, l as DEFAULT_RECONNECT_POLICY, m as sleepWithAbort, n as WhatsAppConnectionController, o as createWaSocket, p as resolveReconnectPolicy, u as computeBackoff } from "./connection-controller-BwKWE-K2.js";
...
import { evaluateSessionFreshness, loadSessionStore, recordSessionMetaFromInbound, resolveChannelResetConfig, resolveSessionKey, resolveSessionResetPolicy, resolveSessionResetType, resolveStorePath, resolveThreadFlag, updateLastRoute, updateSessionStore } from "openclaw/plugin-sdk/session-store-runtime";
	...
	return {
		sendMessage: async (to, text, mediaBuffer, mediaType, sendOptions) => {
			...
			if (mediaBuffer && mediaType?.startsWith("audio/") && text.trim()) {
				...
				if (quotedOpts) await params.sock.sendMessage(jid, textPayload, quotedOpts);
	...
	await sendMediaWithLeadingCaption({
		...
		send: async ({ mediaUrl, caption }) => {
			...
			if (media.kind === "image") {
				...
				if (caption) await sendWithRetry(() => msg.reply(caption, quote), "media:audio-text");
			...
			try {
				connection = await controller.openConnection({
					...
					onWatchdogTimeout: (snapshot) => {
						...
						const watchdogReason = transportSilentMs > messageTimeoutMs ? "transport-inactive" : "app-silent";

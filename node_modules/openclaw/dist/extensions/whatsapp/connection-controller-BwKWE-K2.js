...
import { C as waitForCredsSaveQueueWithTimeout, l as readCredsJsonRaw, s as logoutWeb, v as resolveDefaultWebAuthDir, w as writeCredsJsonAtomically, x as enqueueCredsSave, y as restoreCredsFromBackupIfNeeded } from "./auth-store-Da7U_n_T.js";
...
import { resolveEnvHttpProxyUrl, shouldUseEnvHttpProxyForUrl } from "openclaw/plugin-sdk/infra-runtime";
...
var WhatsAppConnectionController = class {
	...
	startTimers(connection, hooks) {
		...
		connection.watchdogTimer = setInterval(() => {
			...
			if (transportStaleForMs <= this.messageTimeoutMs && appSilentForMs <= this.appSilenceTimeoutMs) return;
			const snapshot = this.getCurrentSnapshot(connection);
			if (!snapshot) return;

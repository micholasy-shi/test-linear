...
import { r as repairCurrentTokenStorageMetaDeviceId } from "./storage-C2Gj7ZHs.js";
...
async function loadMatrixAuthClientDeps() {
	matrixAuthClientDepsPromise ??= Promise.all([import("./sdk-D1tWByPp.js").then((n) => n.n), import("./logging-BwMA7mvv.js").then((n) => n.n)]).then(([sdkModule, loggingModule]) => ({
...
async function loadMatrixCredentialsReadDeps() {
	matrixCredentialsReadDepsPromise ??= import("./credentials-read-BomyXGUF.js").then((n) => n.r).then((credentialsReadModule) => ({
...
async function loadMatrixCredentialsWriteRuntime() {
	matrixCredentialsWriteRuntimePromise ??= import("./credentials-write.runtime-umZYFtCD.js");
	return await matrixCredentialsWriteRuntimePromise;
}
async function loadMatrixSecretInputDeps() {
	matrixSecretInputDepsPromise ??= import("./config-secret-input.runtime-qxMInSAf.js").then((runtime) => ({ resolveConfiguredSecretInputString: runtime.resolveConfiguredSecretInputString }));

import { C as debugLog } from "./sender-C4B08bDl.js";
import { c as getQQBotDataDir, o as getHomeDir, p as isWindows } from "./target-parser-CqH_T7ny.js";
...
*/
function resolveQqbotDownloadsDirForApp(appId) {
	const id = appId.trim();
	if (!id || id.includes("\0") || /[/\\\n]|\.\./.test(id)) throw new Error("invalid appId path");
	const base = path.join(getHomeDir(), ".openclaw", "media", "qqbot", "downloads");
	const resolvedBase = path.resolve(base);
	const target = path.resolve(path.join(resolvedBase, id));
	if (target === resolvedBase || !target.startsWith(resolvedBase + path.sep)) throw new Error("invalid appId path");
	return target;
}
...
function registerStreamingCommands(registry) {
	registry.register({
		...
		handler: async (ctx) => {
			if (ctx.type !== "c2c") return `❌ 流式消息仅支持私聊场景，请在私聊中使用 /bot-streaming 指令`;
			const arg = ctx.args.trim().toLowerCase();

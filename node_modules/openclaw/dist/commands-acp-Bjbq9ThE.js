...
import { a as requireGatewayClientScopeForInternalChannel } from "./command-gates-B8faL7lt.js";
import { C as stopWithText, b as resolveAcpAction, d as COMMAND, x as resolveAcpHelpText } from "./shared-DWgyseFa.js";
...
async function loadAcpActionHandler(action) {
	if (action === "spawn" || action === "cancel" || action === "steer" || action === "close") {
		lifecycleHandlersPromise ??= import("./lifecycle-7P2XfFFr.js");
	...
	if (action === "status" || action === "set-mode" || action === "set" || action === "cwd" || action === "permissions" || action === "timeout" || action === "model" || action === "reset-options") {
		runtimeOptionHandlersPromise ??= import("./runtime-options-DusD_paV.js");
	...
	diagnosticHandlersPromise ??= import("./diagnostics-DIG3MCn9.js");

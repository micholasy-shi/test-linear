...
function installDebugProxyGlobalFetchPatch(settings) {
	...
	if (existing && globalThis.fetch === existing.patchedFetch) return;
	...
	patched[DEBUG_PROXY_FETCH_PATCH_KEY] = {
		originalFetch,
		patchedFetch
	};
...
function uninstallDebugProxyGlobalFetchPatch() {
	...
	if (!state) return;
	if (globalThis.fetch === state.patchedFetch) globalThis.fetch = state.originalFetch;
	delete patched[DEBUG_PROXY_FETCH_PATCH_KEY];
}
function isDebugProxyGlobalFetchPatchInstalled() {
	const state = globalThis[DEBUG_PROXY_FETCH_PATCH_KEY];
